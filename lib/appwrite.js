...
export async function createReservation({
  ...
  try {
    // Extract the day from the date and convert it to an integer
    const day = new Date(date).getDate();

    const newReservationData = {
      resourceType,
      date: day, // Save the day number as an integer
      timeSlot,
...
export const isSlotBooked = async (date, timeSlot, resourceType) => {
  try {
    ...
    if (resourceType === "Gym" || resourceType === "Swimming Pool") {
      return bookings.total >= 20;
    } else {
      return bookings.total >= 2; //badminton and others
    }
  } catch (error) {
